...
import com.cdd.config.filter.JwtAuthenticationTokenFilter;
...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        // 在 UsernamePasswordAuthenticationFilter 之前添加自定义的 jwtAuthenticationTokenFilter。
        // UsernamePasswordAuthenticationFilter 的主要职责是处理基于表单的登录认证。
        // 在处理登录请求时，它会尝试从HTTP请求中获取用户名和密码，并使用它们来执行认证。
        // 如果jwtAuthenticationTokenFilter 中已经设置了有效的Authentication，
        // 那么UsernamePasswordAuthenticationFilter UsernamePasswordAuthenticationFilter 会检测到认证对象已存在，并跳过它的认证逻辑
